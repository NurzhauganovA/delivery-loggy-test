...
def expected() -> dict[str, Any]:
    return {
            ...
            'item_type': 'document',
            'message_for_noncall': 'Здравствуйте, не могли вам дозвониться по поводу '
                        'доставки вашей карты. Скажите, в какое время можем '
                        'вам доставить?',
...
@pytest.fixture
def expected_empty_non_call_message() -> dict[str, Any]:
    return {
            'accepted_delivery_statuses': None,
            'category': None,
            'category_id': None,
...
def item_insert_script() -> str:
    return """
           ...
           VALUES (1, 'Базовая кредитная карта 1', 'document', '00:00', '{planned}', null, 2, true, false, null, false,
                   'Здравствуйте, не могли вам дозвониться по поводу доставки вашей карты. Скажите, в какое время можем вам доставить?', false, null, null);
           INSERT INTO public.item (id, name, item_type, delivery_time, delivery_type, category_id, partner_id,
                                    is_delivery_point_exists, has_postcontrol, accepted_delivery_statuses, distribute,
                                    message_for_noncall, courier_appointed_sms_on, courier_appointed_sms,
                                    days_to_delivery)
