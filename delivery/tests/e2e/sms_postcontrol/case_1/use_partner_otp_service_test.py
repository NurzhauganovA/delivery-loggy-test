...
@patch(
    target='api.services.sms.notification.send_post_control_otp',
    new_callable=AsyncMock,
)
async def test_use_partner_otp_service(
        mock_our_otp_send,
    ...
    assert mock_partner_otp_send.called

    assert not mock_our_otp_send.called
...
@patch(
    target='api.models.order.send_post_control_otp',
    new_callable=AsyncMock,
)
async def test_use_our_otp_service(
    ...
    response = await client.post(
        url='/v1/order/2/sms-postcontrol',
        headers={'Authorization': 'Bearer ' + access_token},
        json={
            'latitude': 11.11,
            'longitude': 22.22,
